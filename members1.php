<?php
	...
	$result = queryMysql("SELCT user FROM members ORDER BY user");
	$num = $result->num_rows;
	...
	for($j = 0; $j < $num; ++$j){
		...
		if(!$t1) echo "[<a href='memebers.php?add=" . $row['user'] . "'>$follow</a>";
		else echo "[<a href='memebers.php?remove=" . $row['user'] . "'>drop</a>]";
	}
?>
	</ul></div>
	</div>
